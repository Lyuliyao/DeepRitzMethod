...
f(x)=2*π^2 .*sum(cos.(π*x[i,:]') for i=1:4, dims=1)
...
function test(N;traintime=2000)
  ...
  evalcb = function()
    ...
    if cntr%500==0
      ...
      CSV.write("$(N)rand4Dexm2.csv",errdf)
      @save "$(N)randpoint4Dexm2m.bson" M
      weights=Tracker.data.(params(M))
      @save "$(N)randpoint4Dexm2w.bson" weights
    end
  end
  θ=Flux.params(M)
  opt=ADAM()
  dataset=[Int64(100*(floor(i/100)+1)*N/traintime) for i=1:traintime]
