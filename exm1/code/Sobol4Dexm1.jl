...
f(x)= π^2 .* sum(cos.(pi*x[i,:]') for i=1:4, dims=1)
...
function lossFD(N)
  ...
  for i=1:4
    ...
    l1[i]=1
    l1 =l1 |>gpu
    losses += (2^4)*(sum(0.5*((F(notes .+ d.*l1   ) .- FFF)./d).^2)/N)[1]
  end
  losses += (2^4)sum((-f(notes) .*FF))/N
  return losses
end
...
function loss_true(xx=1,yy=1,xx1=1,xx2=2)
  ...
  F_true(x)= sum(cos.(pi*x[i,:]') for i=1:4, dims=1)
  errors=sqrt(2^4*sum(((F(points)-F_true(points)).^2)/2000))
  return errors
end
...
function test(N;traintime=2000)
  ...
  evalcb = function()
    ...
    if cntr%500==0
      ...
      CSV.write("$(N)Sobol4Dexm1.csv",errdf)
      @save "$(N)Sobolpoint4Dexm1m.bson" M
      weights=Tracker.data.(params(M))
      @save "$(N)Sobolpoint4Dexm1w.bson" weights
    end
  end
  θ=Flux.params(M)
  opt=ADAM()
